...
public class DirectoryService {
    ...
    public List<DirectoryResponse> getDirectoriesByUser(String username){
        try {
            ...
            List<DirectoryResponse> directories = this.getDirectories();
            for(int i=0;i<=directories.size();i++){
                if(directories.get(i).getOwner().getNome().equals(username)) directoriesByUser.add(directories.get(i));
            }
            if(directories.isEmpty()) throw new DirectoryErrorException("Não foi encontrado nenhum diretório na conta de: " + username, ErrorCodeEnum.DIRE0001.getCode());
